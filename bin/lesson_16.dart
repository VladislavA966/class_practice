...
void main() {
  ...
  one.info();
  Student vlad = Student(name: 'Влад', age: 27, ocenki: [5, 3, 4, 2, 5]);
  vlad.info();
  Auto honda = Auto(marka: 'Honda', model: 'Accord', age: 2003);
  honda.info();
  Book bookOne = Book(name: '1984', writer: 'Джорджем Оруэлом', age: 1949);
  bookOne.info();
  Pryamougolnik odin = Pryamougolnik(height: 15, weight: 25);
  odin.info();
  odin.perimetr();
  odin.ploshad();
  BankAccount userVlad = BankAccount(balance: 500, name: 'Vlad', num: 125365);
  userVlad.info();
  userVlad.popolnenie();
  userVlad.snyatie();
  Product phone = Product(name: 'Iphone 14', price: 1000, count: 12);
  phone.info();
  phone.allValue();
  Employee dev = Employee(name: 'Vlad', position: 'Junior', salary: 600);
  dev.info();
  dev.salaryUpd(12);
